...
const GET = async ({ url, locals }) => {
  const id_level = url.searchParams.get("id");
  const search = url.searchParams.get("search");
  const token = locals.token;
  if (!token) return json({ message: "Unauthorized" }, 401);
  var endpoint = "";
  if (search && search != "") {
    endpoint = `${apiUrl}/cari_level?search=${search}`;
  } else {
    endpoint = id_level ? `${apiUrl}/level/${id_level}` : `${apiUrl}/level`;
  }
...
const PUT = async ({ url, request, locals }) => {
  ...
  const endpoint = `${apiUrl}/level/${id_level}`;
  const res = await fetch(endpoint, {
    method: "PUT",
    headers: { Authorization: `${token}` },
    body: formData
