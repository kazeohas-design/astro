...
const GET = async ({ url, locals }) => {
  ...
  if (search && search != "" || dari_tanggal && dari_tanggal != "" && sampai_tanggal && sampai_tanggal != "") {
    endpoint = page == "permohonan" ? `${apiUrl}/cari_riwayat?search=${search}&dari_tanggal=${dari_tanggal}&sampai_tanggal=${sampai_tanggal}` : `${apiUrl}/cari_riwayat_layanan?search=${search}&dari_tanggal=${dari_tanggal}&sampai_tanggal=${sampai_tanggal}`;
  } else {
    if (id_trash != "") {
      endpoint = page == "permohonan" ? `${apiUrl}/riwayat_permohonan/${id_trash}` : `${apiUrl}/riwayat_layanan/${id_trash}`;
    } else {
      endpoint = page == "permohonan" ? `${apiUrl}/riwayat_permohonan` : `${apiUrl}/riwayat_layanan`;
    }
  }
  console.log("endpoint riwayat", endpoint);
...
const DELETE = async ({ request, locals }) => {
  ...
  var riwayat_link = "";
  if (page == "permohonan") {
    riwayat_link = `${apiUrl}/riwayat_permohonan`;
  } else {
    riwayat_link = `${apiUrl}/riwayat_layanan`;
  }
  const res = await fetch(riwayat_link, {
    method: "POST",
    headers: {
      Authorization: `${token}`,
